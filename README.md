...
- **SEO/GEO** — every page ships canonical + Open Graph/Twitter meta and JSON-LD (`WebSite`/`Organization` on the home page; `WebApplication` + `BreadcrumbList` + `FAQPage` per tool, with matching visible FAQ sections). `public/` holds `robots.txt`, `sitemap.xml`, `llms.txt` (AI-crawler site overview), `og.png` (1200×630) and `apple-touch-icon.png` — the two images are rendered from HTML via headless Chrome.
