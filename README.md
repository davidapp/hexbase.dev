...
- **Metrics** — aggregate page views (path + country, nothing else) via Workers Analytics Engine: HTML routes are in `run_worker_first`, the worker's `notFound` fallthrough calls `writeDataPoint` and serves the asset. No client-side analytics script exists. Query with the [Analytics Engine SQL API](https://developers.cloudflare.com/analytics/analytics-engine/sql-api/) against the `hexbase_metrics` dataset.
